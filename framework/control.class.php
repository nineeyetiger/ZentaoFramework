...
class control
{
    ...
    /**
     * 数据直接放在'data'字段中，不再用自定义变量名封装。
     * @param $value
     */
    public function assignData($value)
    {
    	$this->view->linglingData = $value;
    }
    ...
    private function parseJSON($moduleName, $methodName)
    {
        ...
        if(isset($this->view->linglingData)){
        	$output['data']   = json_encode($this->view->linglingData);	
        }else{
        	$output['data']   = json_encode($this->view);
        }
        // 不需要MD5 $output['md5']    = md5(json_encode($this->view));
