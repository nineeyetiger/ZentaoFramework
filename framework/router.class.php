...
class router
{
    ...
    {
    	// 看似相对路径，实则绝对路径，提前已经假定当前文件在根目录的framework子目录中，所以连用两个dirname。
    ...
    {
    	// $config->moduleVar   = 'm'; $config->methodVar   = 'f';
    	// 访问根目录时，执行入口文件：index.php。没有设置moduleVar和methodVar，所以使用默认module和method：
    	// $config->default->module = 'index'; $config->default->method = 'index';
        ...
        // 到此处，module、method和controlFile的位置都已经确定。
    }
    ...
    public function loadModule()
    {
        ...
        // 通过导入“moduleName/ext/control/methodName.php”文件，支持对模块的默认method进行扩展。
        $file2Included = $this->setActionExtFile() ? $this->extActionFile : $this->controlFile;
        // TODO 这是个绝对路径，为什么还要跳转过去后再include呢？
        chdir(dirname($file2Included));
        include $file2Included;

        /* Set the class name of the control. */
        // 上一步include的控制文件中，必须包含有名为my$moduleName或$moduleName的类。
        ...
        // 由于参数表是可变，并不可预估的，为了统一处理，所以使用这种形式，类似于C中的...参数。
    }
